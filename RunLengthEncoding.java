...
public class RunLengthEncoding implements Iterable{
  ...
  public RunLengthEncoding(int width, int height) {
    // Your solution here.
	  ...
	  this.width = width;
	  this.height = height;
	  int[] RGBNode = new int[4];
	  RGBNode[0]=size;
	  RGBNode[1]=0;
	  RGBNode[2]=0;  
	  RGBNode[3]=0;
	  RGB = new SList();
	  RGB.insertEnd(RGBNode);
  ...
  public RunLengthEncoding(int width, int height, int[] red, int[] green,
    ...
    // Your solution here.
	  ...
	  size = width*height;
	  RGB = new SList();
	  if(red.length == green.length && blue.length == runLengths.length && green.length == blue.length){
		  for(int i=0;i<red.length;i++){
			  int[] RGBNode = new int[4];
			  RGBNode[1] = red[i];
			  RGBNode[2] = green[i];
			  RGBNode[3] = blue[i];
			  RGBNode[0] = runLengths[i];
			  RGB.insertEnd(new SListNode(RGBNode));
	  }
	 } 
  }
  ...
  public RunIterator iterator() {
    // Replace the following line with your solution.
	RunIterator i = new RunIterator(RGB.getHead());
    return i;
  ...
  public PixImage toPixImage() {
	...
	int position = -1;
	PixImage newPic = new PixImage(this.getWidth(),this.getHeight());
	while(it.hasNext()){
		int[] temp = new int[4];
		for(int i=0;i<4;i++){
			temp[i] = ((int[])it.getItem())[i];
		}
		//System.out.println(Arrays.toString(temp));
		int num = temp[0];
		position += temp[0];
		for(int i=0;i<num;i++){
			newPic.setPixel((position-num+i+1)%width, (position-num+i+1)/width, (short)temp[1], (short)temp[2], (short)temp[3]);
		}
		it.next();
	}
	int[] temp = new int[4];
  ...
  public String toString() {
	...
	for(int i=1;i<=RGB.length();i++){
		s = s.concat("| "+((int[])RGB.nth(i))[1]+"+"+((int[])RGB.nth(i))[2]+"+"+((int[])RGB.nth(i))[3]+" , "+((int[])RGB.nth(i))[0]+" |");
	}
    return s;
  }
  ...
  public RunLengthEncoding(PixImage image) {
	...
	RGB = new SList();
	SList total = new SList();
	SList totalcopy = new SList();
	for(int i=0;i<height;i++){
		for(int j=0;j<width;j++){
			int[] ele = {image.getRed(j, i), image.getGreen(j, i), image.getBlue(j, i)};
			total.insertEnd(ele);
			totalcopy.insertEnd(ele);
		}
	}
	totalcopy.squishInt();
	//System.out.println(total);
	//System.out.println(totalcopy);
	while(totalcopy.getHead()!=null){
		int num =0;
		while(true){
			if(Arrays.equals((int[])total.nth(1), (int[])totalcopy.nth(1))){
				total.deleteFront();
				num++;
			}else{
				int[] temp = (int[])totalcopy.getHead().item;
				//System.out.println(Arrays.toString(temp));
				int[] temp1 = new int[4];
				temp1[0] = num;
				temp1[1] = temp[0];
  ...
  public void check() {
    // Your solution here.
	  ...
	  int sum =0;
	  while(it.hasNext()){
		  sum += ((int[])it.getItem())[0];
		  if(((int[])it.getItem()).equals(it.next())){
			  System.out.println("There are two consecutive runs!");
		  }
	  }
	  sum += ((int[])it.getItem())[0];
	  if(sum != size){
  ...
  public void setPixel(int x, int y, short red, short green, short blue) {
    // Your solution here, but you should probably leave the following line
    //   at the end.
	PixImage image = this.toPixImage();
	image.setPixel(x, y, red, green, blue);
	RunLengthEncoding temp = new RunLengthEncoding(image);
	this.RGB = new SList();
	this.RGB.copy(temp.getRGB());
    check();
  }
